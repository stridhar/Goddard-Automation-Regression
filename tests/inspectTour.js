...
(async () => {
  ...
  const elements = await page.locator('button, a, [role="button"], [role="link"], input, [aria-label]').evaluateAll((els) => els.map((el) => ({
  ...
  console.log(JSON.stringify(elements.filter((x) => x.ariaLabel || x.text?.includes('PM') || x.text?.includes('AM') || x.text?.includes('Date') || x.text?.includes('Time')).slice(0, 400), null, 2));
  await browser.close();
})().catch((err) => {
  console.error(err);
  process.exit(1);
